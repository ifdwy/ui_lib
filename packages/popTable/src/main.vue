...
export default {
  ...
  methods: {
    ...
    // 行拖拽
    rowDrop() {
      this.sortable = Sortable.create(this.$refs.xTable1.$el.querySelector('.body--wrapper>.vxe-table--body tbody'), {
        handle: '.drag-btn',
        onEnd: (sortableEvent) => {
          const newIndex = sortableEvent.newIndex;
          const oldIndex = sortableEvent.oldIndex;
    ...
    selectAllEvent({ checked, records }) {
      this.$listeners.selectAllEvent({ checked, records });
    },
    selectChangeEvent({ checked, records }) {
      this.$listeners.selectChangeEvent({ checked, records });
    },
  },
  destroyed() {
